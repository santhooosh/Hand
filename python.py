...
def select():
    ...
    message = tk.Label(root, text="" ,bg="#7DCEA0"  ,fg="#7DCEA0"  ,width=int((0.0462962)*screen_height)  ,height=3,font=('caliber', int((0.027777)*screen_height), 'bold')) 
...
def start(doc,doc1,name):
    ...
    doc.Close()
    word.Quit()
    message = tk.Label(root, text="Successfully converted to "+name+".pdf" ,bg="green"  ,fg="white"  ,width=int((0.0462962)*screen_height)  ,height=3,font=('caliber', int((0.027777)*screen_height), 'bold')) 
...
message = tk.Label(root, text="Document to Handwriting" ,bg="#5DADE2"  ,fg="white"  ,width=int((0.0462962)*screen_height)  ,height=3,font=('caliber', 30, 'bold')) 
message.place(x=(0.20916*screen_width),y=(0.0185185)*screen_height)
space = tk.Button(root, text="click to start", command=select  ,fg="white"  ,bg="#2E86C1"  ,width=int((0.0185185)*screen_height)  ,height=3, activebackground = "white" ,font=('times', 15, ' bold '))
